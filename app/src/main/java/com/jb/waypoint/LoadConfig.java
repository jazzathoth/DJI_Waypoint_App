...
public final class LoadConfig {
    ...
    private static WeakReference<LoadConfig> sInstance = new WeakReference<>(null);
...
//    private Uri mUserInfoEndpointUri;
    ...
    public static LoadConfig getInstance(Context context) {
        LoadConfig config = sInstance.get();
        if (config == null) {
            config = new LoadConfig(context);
            sInstance = new WeakReference<LoadConfig>(config);
        }
...
//    }
    ...
    private void readConfiguration() throws InvalidConfigurationException {
        BufferedSource configSource =
                Okio.buffer(Okio.source(mResources.openRawResource(R.raw.okta_oidc_config)));
...
//            mUserInfoEndpointUri = getRequiredConfigWebUri("user_info_endpoint_uri");
            ...
            if (mClientId == null) {
                mRegistrationEndpointUri =
                        getRequiredConfigWebUri("registration_endpoint_uri");
            }
        } else {
